import { Play, Film, Gift } from "lucide-react";
...
export const VideoLibrary = () => {
    ...
    const { assets } = useTetEvent();

    const myVideos = (assets || []).filter(a => a.asset_type === 'Video');

    const handleAddVideo = (video, isOwned = false) => {
        addElement("video", {
            src: video.url || (isOwned ? video.display?.image_url : null),
            thumbnail: video.thumbnail || (isOwned ? (video.url || video.display?.image_url) : null),
            width: 320,
            height: 180,
            autoPlay: true,
            loop: true,
    ...
    return (
        <div className="space-y-6">
            {/* Owned Videos Section */}
            {myVideos.length > 0 && (
                <div className="space-y-2">
                    <div className="flex items-center gap-2">
                        <Gift className="w-3 h-3 text-red-500" />
                        <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500">My Videos</h4>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        {myVideos.map((video) => (
                            <button
                                key={video.id}
                                onClick={() => handleAddVideo(video, true)}
                                className="cursor-pointer group relative aspect-video rounded-xl overflow-hidden border border-gray-200 dark:border-white/10 hover:border-purple-500 transition-all shadow-sm active:scale-95 bg-black"
                            >
                                <video src={video.url || video.display?.image_url} className="w-full h-full object-cover opacity-80 group-hover:opacity-100" />
                                <div className="absolute top-1 right-1">
                                    <span className="text-[8px] bg-red-600 text-white px-1 rounded">OWNED</span>
                                </div>
                            </button>
                        ))}
                    </div>
            ...
            {/* Default Library */}
            <div className="space-y-2">
                <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500">Stock Videos</h4>
                <div className="grid grid-cols-2 gap-3">
