...
console.log('🔄 Starting Global Config Synchronization...');
...
try {
    if (fs.existsSync(PUBLISHED_TOML_PATH)) {
        ...
        if (match && match[1]) {
            packageId = match[1];
            console.log(`✅ Found Package ID in Published.toml: ${packageId}`);
        } else {
            console.warn('⚠️ Could not parse original-id from Published.toml');
        }
    } else {
        console.warn(`⚠️ Published.toml not found at: ${PUBLISHED_TOML_PATH}`);
    }
} catch (error) {
    console.error('❌ Error reading Published.toml:', error);
}
...
try {
    if (fs.existsSync(GLOBAL_CONFIG_PATH)) {
        ...
        console.log('✅ Loaded global.config.json');
    }
} catch (error) {
    console.error('❌ Error reading global.config.json:', error);
}
...
try {
    ...
    console.log(`✅ Updated client/.env successfully!`);
